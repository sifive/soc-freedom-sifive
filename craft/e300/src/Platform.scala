...
class E300ArtyDevKitPlatform(implicit val p: Parameters) extends Module {
  ...
  sys.iofs.head.foreach { sink =>
    sink.bundle.iof_0.foreach { _.default() }
    sink.bundle.iof_1.foreach { _.default() }
  }

  //-----------------------------------------------------------------------

  sys.iofs.head.foreach { sink =>
    val iof_0 = sink.bundle.iof_0
    val iof_1 = sink.bundle.iof_1
